...
cat "$T_FOLDER"/d/dblank.txt | c/merge.js "$T_FOLDER"/d/m4.txt > d/temp-global-index.txt
mv d/temp-global-index.txt "$T_FOLDER"/d/output.txt

if DIFF_PERCENT=$DIFF_PERCENT t/gi-diff.js <(sort "$T_FOLDER"/d/output.txt) <(sort "$T_FOLDER"/d/m4.txt) > /dev/null;
then
